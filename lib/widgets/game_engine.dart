...
class _GameEngineState extends State<GameEngine> with SingleTickerProviderStateMixin {
  ...
  Widget _buildGameScreen() {
    ...
    switch (qType) {
      ...
      case 'compareSize':
        return _buildCompareSizeScreen(level);
      case 'compareQuantity':
        return _buildCompareQuantityScreen(level);
      case 'completeHalf':
        return _buildCompleteHalfScreen(level);
      case 'completeShape':
  ...
  // ========== COMPARE SIZE ==========
  Widget _buildCompareSizeScreen(GameLevel level) {
    final emojis = level.emojis;
    final answerIdx = int.tryParse(level.answer) ?? 0;
    return SafeArea(
      child: Column(
        children: [
          _buildHeader(),
            ...
            child: Padding(
              ...
              child: Wrap(
                ...
                children: List.generate(emojis.length, (i) {
                  final isSelected = _selectedOptions.contains(i.toString());
                  final sizes = [40.0, 56.0, 72.0, 88.0, 104.0];
                  final emojiSize = sizes[i.clamp(0, sizes.length - 1)];
                  return GestureDetector(
                    onTap: () {
                      _playSound();
                      setState(() { _selectedOptions = [i.toString()]; });
                      if (i == answerIdx) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Not the right one!'), backgroundColor: Colors.red, duration: Duration(seconds: 1)),
                        );
                    ...
                    child: AnimatedContainer(
                      ...
                      width: 80, height: 80,
                      decoration: BoxDecoration(
                        color: isSelected ? _getColor('primary')!.withOpacity(0.2) : const Color(0xFF21262D),
                        borderRadius: BorderRadius.circular(14),
                        border: Border.all(
                          color: isSelected ? _getColor('primary')! : Colors.grey.shade700,
                          width: isSelected ? 2.5 : 1,
                        ),
                      ),
                      child: Center(child: FittedBox(fit: BoxFit.scaleDown, child: Text(emojis[i], style: TextStyle(fontSize: emojiSize)))),
                    ),
                  );
                }).toList(),
              ),
  ...
  // ========== COMPARE QUANTITY ==========
  Widget _buildCompareQuantityScreen(GameLevel level) {
    final groups = level.emojis;
    final answerIdx = int.tryParse(level.answer) ?? 0;
    return SafeArea(
      child: Column(
        children: [
                  ...
                  return GestureDetector(
                    onTap: () {
                      ...
                      if (i == answerIdx) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Not quite!'), backgroundColor: Colors.red, duration: Duration(seconds: 1)),
                        );
                        setState(() => _selectedOptions = []);
                      }
                    },
                    child: AnimatedContainer(
                      ...
                      child: Center(
                        child: FittedBox(
                          ...
                          child: Text(groups[i], style: const TextStyle(fontSize: 28)),
                        ),
                      ),
                    ),
                  );
  ...
  // ========== COMPLETE HALF ==========
  Widget _buildCompleteHalfScreen(GameLevel level) {
    final emojis = level.emojis;
    if (emojis.isEmpty) return _emptyScreen();
    final leftHalf = emojis[0];
    final options = emojis.sublist(1);
    return SafeArea(
      child: Column(
        children: [
          _buildHeader(),
          ...
          Container(
            ...
            child: Center(child: Text(leftHalf, style: const TextStyle(fontSize: 80))),
          ),
          const SizedBox(height: 24),
          const Text('Choose the matching half:', style: TextStyle(color: Colors.grey, fontSize: 13)),
          const SizedBox(height: 12),
          Expanded(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
                  ...
                  return GestureDetector(
                    onTap: () {
                      ...
                      if (options[i] == level.answer) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Wrong half!'), backgroundColor: Colors.red, duration: Duration(seconds: 1)),
                        );
                        setState(() => _selectedOptions = []);
                      }
                    },
                    child: Container(
                      ...
                      child: Center(child: FittedBox(fit: BoxFit.scaleDown, child: Text(options[i], style: const TextStyle(fontSize: 50)))),
                    ),
                  );
                }).toList(),
              ),
  ...
  // ========== COMPLETE SHAPE ==========
  Widget _buildCompleteShapeScreen(GameLevel level) {
    final emojis = level.emojis;
    if (emojis.isEmpty) return _emptyScreen();
    final mainEmoji = emojis[0];
    final options = emojis.sublist(1);
    return SafeArea(
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Stack(
              ...
              children: [
                Text(mainEmoji, style: const TextStyle(fontSize: 80)),
                Container(
                  width: 40, height: 40,
                  decoration: BoxDecoration(
                    color: _getColor('background'),
          ...
          const SizedBox(height: 24),
          const Text('Choose the correct shape:', style: TextStyle(color: Colors.grey, fontSize: 13)),
          const SizedBox(height: 12),
          Expanded(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
                  ...
                  return GestureDetector(
                    onTap: () {
                      ...
                      if (options[i] == level.answer) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Wrong shape!'), backgroundColor: Colors.red, duration: Duration(seconds: 1)),
                        );
                        setState(() => _selectedOptions = []);
                      }
                    },
  ...
  // ========== FILL SEQUENCE ==========
  Widget _buildFillSequenceScreen(GameLevel level) {
    final sequence = level.emojis;
    final available = level.options;
    if (_lastFillSequenceLevelIndex != _internalLevelIndex || _fillSequenceAvailable.isEmpty) {
      _fillSequenceAvailable = List<String>.from(available)..shuffle(_random);
      _lastFillSequenceLevelIndex = _internalLevelIndex;
      _selectedOptions = List<String>.filled(sequence.where((s) => s == '?').length, '');
    }
    final blanks = <int>[];
    for (int i = 0; i < sequence.length; i++) {
      if (sequence[i] == '?') blanks.add(i);
    }
    final filledCount = _selectedOptions.where((s) => s.isNotEmpty).length;
    final allFilled = filledCount == blanks.length;
    int currentBlankIdx = 0;
    for (int i = 0; i < blanks.length; i++) {
      if (_selectedOptions[i].isEmpty) { currentBlankIdx = i; break; }
      if (i == blanks.length - 1) currentBlankIdx = blanks.length - 1;
    }
    return SafeArea(
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Wrap(
              ...
              children: List.generate(sequence.length, (i) {
                if (sequence[i] == '?') {
                  final blankIdx = blanks.indexOf(i);
                  final val = blankIdx < _selectedOptions.length ? _selectedOptions[blankIdx] : '';
                  return Container(
                    width: 36, height: 36,
                    decoration: BoxDecoration(
                      color: val.isNotEmpty ? _getColor('primary')!.withOpacity(0.3) : const Color(0xFF30363D),
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(
                        color: val.isNotEmpty ? _getColor('primary')! : Colors.grey.shade600,
                      ),
                    ),
                    child: Center(
                      child: val.isNotEmpty
                          ? GestureDetector(
                              onTap: () {
                                setState(() {
                                  _selectedOptions[blankIdx] = '';
                                  _fillSequenceAvailable.add(val);
                                });
                              },
                ...
                return Container(
                  ...
                  child: Center(child: Text(sequence[i], style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16))),
                );
              }),
            ),
          ),
          const SizedBox(height: 20),
          const Text('Tap to fill the blanks:', style: TextStyle(color: Colors.grey, fontSize: 12)),
          const SizedBox(height: 10),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Wrap(
              spacing: 8, runSpacing: 8, alignment: WrapAlignment.center,
              children: _fillSequenceAvailable.map((item) {
                return GestureDetector(
                  onTap: () {
                    if (filledCount >= blanks.length) return;
                    _playSound();
                    setState(() {
                      _fillSequenceAvailable.remove(item);
                      for (int b = 0; b < blanks.length; b++) {
                        if (_selectedOptions[b].isEmpty) {
                          _selectedOptions[b] = item;
                          break;
                        }
                      }
                    });
                    final nowFilled = _selectedOptions.where((s) => s.isNotEmpty).length;
                    if (nowFilled == blanks.length) {
                      bool allCorrect = true;
                      for (int b = 0; b < blanks.length; b++) {
                        if (_selectedOptions[b] != level.answer) { allCorrect = false; break; }
                      }
                      if (allCorrect) {
                        Future.delayed(const Duration(milliseconds: 200), () {
                          if (mounted) {
                        ...
                        setState(() {
                          _fillSequenceAvailable = List<String>.from(available)..shuffle(_random);
                          _selectedOptions = List<String>.filled(blanks.length, '');
                        });
                      }
                    }
                  },
                  child: Container(
                    ...
                    child: Center(child: Text(item, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18))),
                  ),
                );
              }).toList(),
            ),
